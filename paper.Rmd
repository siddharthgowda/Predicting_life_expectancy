...
Life expectancy is a fundamental indicator of a country's overall health and development. It reflects the effectiveness of healthcare systems, economic stability, and social progress. In developing countries, healthcare resources are often constrained and economic disparities persist. Understanding the factors that influence life expectancy is critical for shaping effective policies. This study examines how economic, health, and social factors influence life expectancy in developing countries, collectively and individually. 

A multiple linear regression model is used, incorporating alcohol consumption (categorical variable), schooling, infant mortality, health expenditure (as a percentage of a country’s GDP per capita), and HIV deaths as explanatory variables. We also included GDP per capita in the preliminary model but later removed it from the final set of predictors. Existing literature provides strong evidence supporting the relationship between these factors and life expectancy. Miladinov (2020) examined socioeconomic development and life expectancy in EU accession candidate countries, finding that higher GDP per capita and lower infant mortality were associated with increased life expectancy. 

Our study focuses on developing nations facing similar economic and healthcare challenges. Adebayo et al. (2024) analyzed U.S. data, finding that higher health expenditure and GDP per capita positively impact life expectancy, while higher infant mortality reduces it. Our study extends this analysis to developing countries, comparing how these determinants function in different economic contexts. A third study on OECD countries (Roffia et al., 2022) found that education and economic conditions played a critical role in increasing life expectancy. 

This supports schooling as a key predictor, as education directly influences health outcomes and economic mobility. To analyze these relationships, we employ multiple linear regression, allowing us to examine the relationship between life expectancy and multiple predictor variables simultaneously. Life expectancy is influenced by a combination of factors, making a univariate approach insufficient for capturing this complexity. This enables us to quantify the individual impact of each variable while controlling for others. This study seeks to generate actionable insights for policymakers to improve life expectancy in developing nations.
...
kable(model_table, digits = 4, align = "c",
      col.names = c("Variable", "Estimate", "Std. Error", "Pr(>|t|)"))
```
...
Interestingly, our model revealed an unexpected negative coefficient (-1.511) associated with countries categorized as having negligible alcohol consumption, indicating that these countries, on average, have approximately 1.51 years lower life expectancy compared to those with regular alcohol consumption, assuming all other variables are held constant. This counterintuitive result contrasts with the existing literature, where higher alcohol consumption is typically linked to adverse health outcomes and lower longevity (Roffia et al., 2023). It suggests that in developing countries, negligible alcohol consumption may reflect broader socio-cultural, economic, or policy contexts, such as restrictive laws or limited social freedoms, potentially indirectly correlating with restricted healthcare access or lower socioeconomic status. This finding indicates a need for deeper qualitative investigation to understand the broader social determinants involved and their indirect effects on life expectancy.

It is important to note a deviation from previous literature regarding GDP per capita, which was ultimately excluded from our final model due to statistical insignificance. Prior studies consistently demonstrated GDP per capita as a strong predictor positively associated with life expectancy (Miladinov, 2020; Adebayo et al., 2024). However, in our model, the relationship between GDP per capita and life expectancy might have been captured indirectly through other explanatory variables like schooling and healthcare expenditure. These variables may effectively mediate the economic benefits usually associated with GDP, highlighting the importance of explicitly identifying and understanding such mediating mechanisms in developing countries.
...
results <- data.frame(
  Term = c("Intercept", "log(1 + Infant Deaths)", "log(HIV/AIDS Deaths + 1)", "schooling", 
           "log(1 + Health Expenditure)", "alcoholNegligible"),
...
kable(results, col.names = c("Term", "Coefficent Estimates", "Std. Error", "Pr(>|t|)", 
...
hist(rstandard(secondary_model_infant_lg),  main="Histogram of Standized Residuals", xlab="Residuals")
```
...
Hasan: Wrote for section 1,2,3 and 6. Conducted research and constructed the research question. Created and interpreted key findings for Infant deaths, HIV/AIDs and health expenditure. Created policy recommendations.

Arshh: Contributed writing to Section 1, 2 and 3. Wrote section 5, helped with interpretation of final model results and model transformations, conducted research and literature review, helped formulate research question and helped with formatting
